...
| Add fixture/golden example or change renderer/MRZ fixture semantics | [Fixtures and evaluation](quality/fixtures-and-evaluation.md) | `fixtures/catalogue.py`, `fixtures/identifiers.py`, `fixtures/render.py`, `fixtures/generate.py`, `eval/run_eval.py` | `EXAMPLES`, `Example`, `Submission`, `Row`, `Table`, `Mrz`, `AmbiguousTransliteration`, `FixtureDoesNotFit` | `tests/test_fixture_renderer.py`; paid eval is conditional | `uv run pytest tests/test_fixture_renderer.py` |
| Collect a Számlázz.hu demo invoice reference for fixture/layout research | [Fixtures and evaluation](quality/fixtures-and-evaluation.md#external-reference-collection) | `scripts/issue_demo_invoice_wizard.sh` | `ENV_FILE`, `DEMO_URL`, `ISSUER_TAX`, `WATERMARK`, `PDF_PATH`; placeholder-issuer gate | shell syntax plus manual stage dry run | `bash -n scripts/issue_demo_invoice_wizard.sh` |

...
- [Synthetic fixtures and golden evaluation](quality/fixtures-and-evaluation.md) — co-derived test artifacts, synthetic reference workflows, and paid real-provider accuracy reporting.
...
| Add fixture/golden example, change renderer/identifiers, or investigate accuracy | [Fixtures and evaluation](quality/fixtures-and-evaluation.md) | `fixtures/catalogue.py`, `fixtures/generate.py`, `fixtures/render.py`, `fixtures/identifiers.py`, `eval/run_eval.py` | `EXAMPLES`, `Example`, `Submission`, `Row`, `Table`, `Mrz`, `FixtureDoesNotFit`, `AmbiguousTransliteration` | `tests/test_fixture_renderer.py`; paid eval is conditional | `uv run pytest tests/test_fixture_renderer.py` |
| Collect or update an external invoice layout reference | [Fixtures and evaluation](quality/fixtures-and-evaluation.md#external-invoice-layout-reference-workflow) | `scripts/issue_demo_invoice_wizard.sh`, `fixtures/catalogue.py` | wizard stages, synthetic buyer data, generated fixture surfaces | no automated runtime test; fixture drift tests only if committed artifacts change | `bash -n scripts/issue_demo_invoice_wizard.sh` |

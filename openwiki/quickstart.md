...
- [Synthetic fixtures and golden evaluation](quality/fixtures-and-evaluation.md) — co-derived test artifacts, identifier/MRZ fixture rules, manual reference workflows, and paid real-provider accuracy reporting.
...
| Add fixture/golden example, synthetic identifier, MRZ value, or investigate accuracy | [Fixtures and evaluation](quality/fixtures-and-evaluation.md) | `fixtures/catalogue.py`, `fixtures/identifiers.py`, `fixtures/render.py`, `fixtures/generate.py`, `eval/run_eval.py` | `EXAMPLES`, `Example`, `Submission`, `Row`, `Table`, `Mrz`, `Geometry`, `FixtureDoesNotFit`, `AmbiguousTransliteration`, `mrz_td1`, `mrz_td3` | `tests/test_fixture_renderer.py`; paid eval is conditional | `uv run pytest tests/test_fixture_renderer.py` |
...
| Use or update the Számlázz.hu demo invoice reference workflow | [Fixtures and evaluation](quality/fixtures-and-evaluation.md#manual-reference-artifact-workflow) | `scripts/issue_demo_invoice_wizard.sh`, `fixtures/catalogue.py` | `issue_demo_invoice_wizard.sh`, `ENV_FILE`, `invoice/happy_path` | Manual browser gates; no deterministic pytest target | `bash scripts/issue_demo_invoice_wizard.sh` |
